    ...
    <div className="view-enter">
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 24, alignItems: "center" }}>
        <button onClick={() => setFilter(null)}
          className={"chip" } style={{ cursor: "pointer", borderColor: !filter ? "var(--ink)" : "var(--line-2)",
            background: !filter ? "var(--ink)" : "transparent", color: !filter ? "var(--ink-rev)" : "var(--ink-2)" }}>
          全部 {D.members.length}
        </button>
        {roles.map((r) => (
          <button key={r} onClick={() => setFilter(filter === r ? null : r)} className="chip"
            style={{ cursor: "pointer", borderColor: cvarOf(r),
              background: filter === r ? cvarOf(r) : "transparent", color: filter === r ? "#fff" : cvarOf(r) }}>
...
function FeatureRow({ k, member, members }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "118px 1fr 92px 70px", alignItems: "center", gap: 14,
      padding: "9px 0", borderBottom: "1px solid var(--line)" }}>
      <span style={{ fontSize: 13.5, color: "var(--ink-2)" }}>{meta.label}</span>
      <div style={{ height: 7, background: "var(--paper-2)" }}>
        <div style={{ height: "100%", width: pct + "%", background: cvarOf(member.role) }} />
      </div>
      <span className="num" style={{ fontSize: 15, textAlign: "right" }}>{fmtVal(k, val)}</span>
...
/* ---------- 成員詳細 ---------- */
function DetailView({ memberId, onOpenMember, goMembers }) {
  const D = window.APP_DATA;
    ...
    <div className="view-enter">
      ...
      <div className="panel" style={{ borderTop: `4px solid ${cvarOf(m.role)}`, marginBottom: 18 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 0 }}>
          <div style={{ padding: "30px 32px", borderRight: "1px solid var(--line)" }}>
            <RoleTag role={m.role} solid />
            <div style={{ display: "flex", gap: 22, alignItems: "center", margin: "22px 0 18px" }}>
              <Seal role={m.role} size={96} />
              <div>
                <div className="serif" style={{ fontSize: 44, fontWeight: 700, lineHeight: 1, color: cvarOf(m.role) }}>{meta.title}</div>
                <div style={{ fontSize: 20, fontWeight: 600, marginTop: 8 }}>{m.name}</div>
              </div>
            </div>
            <p className="serif" style={{ fontSize: 21, lineHeight: 1.45, margin: "0 0 14px" }}>「{m.tagline}」</p>
            <p style={{ fontSize: 14.5, color: "var(--ink-2)", lineHeight: 1.6, margin: 0 }}>{meta.desc}</p>

            <div style={{ display: "flex", gap: 28, marginTop: 24, paddingTop: 18, borderTop: "1px solid var(--line)" }}>
          ...
          <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", justifyContent: "center", background: softOf(m.role) + "" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 1, background: "var(--line)",
        ...
        {m.top.map((k, i) => {
          ...
          return (
            <div key={k} style={{ background: "var(--surface)", padding: "18px 22px" }}>
...
/* ---------- 圖表 ---------- */
function ChartsView({ onOpenMember }) {
  const D = window.APP_DATA;
    ...
    <div className="view-enter">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.15fr 1fr", gap: 18, marginBottom: 18 }}>
        ...
        <div className="panel">
          ...
          <div className="panel-pad" style={{ display: "grid", gridTemplateColumns: "180px 1fr", gap: 16, alignItems: "center" }}>
            <Donut data={donutData} size={180} thickness={26} onHover={setHover} />
      ...
      <div className="panel" style={{ marginBottom: 18 }}>
        ...
        <div className="panel-pad"><Heatmap data={D.heatmap} onPick={onOpenMember} /></div>
      </div>
      ...
      <div className="panel">
        ...
        <div className="panel-pad" style={{ display: "grid", gridTemplateColumns: "1fr 240px", gap: 24, alignItems: "center" }}>
